...
run_name = f"new_dataset_test_final"
...
def main():
    ...
    # Check which patients exist in your targets CSV
    ...
    # We only split the LABELED patients. Unlabeled go 100% to Train.
    ...
    # Set labels to NaN for all validation subjects to prevent leakage
    # (Assuming level 0 is RegistrationCode)
    val_mask = targets_train.index.get_level_values(0).isin(final_val_subs)
    targets_train.loc[val_mask, 'age'] = np.nan


    # Train Keys: Simply grab all images for the training patients
    train_keys = []
    for s in final_train_subs:
        train_keys.extend(subject_map[s])

    # Val Keys: Must be strictly filtered for valid targets
    val_keys = []
    ...
    # ---------------------------------------------------------
    # Use the corrected class from the previous step
    ...
    train_dataset = LeJEPAHDF5Dataset(
        ...
        transform=train_aug,  # Pass the whole object; Dataset should call .global_trans etc.
    ...
    # ---------------------------------------------------------
    # 7. MODEL & LOOP (Standard)

    encoder = LeJEPA_Encoder(config["model_name"], img_size=config['img_size'], proj_out_dim=64,
                             pretrained=False, drop_path_rate=config['drop_path']).to(DEVICE)
    ...
    for epoch in range(start_epoch, config["epochs"] + 1):
        ...
        for i, (views, age_targets) in enumerate(train_loader):
            ...
            if label_mask.sum() > 0:
                ...
                feat_pooled = g_feats_reshaped.mean(dim=0).detach() # STOP GRADIENT
        ...
        with torch.no_grad():
            for views, age_targets in val_loader:
                ...
                # Val dataset is already filtered, but good to be safe
                age_targets = age_targets.to(DEVICE).view(-1)
